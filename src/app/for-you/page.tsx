...
function BookCard({ book, isPremiumUser }: { book: Book; isPremiumUser: boolean }) {
  return (
    <Link href={`/book/${book.id}`} style={{ textDecoration: "none", flexShrink: 0, display: "block" }}>
      <div
        style={{ display: "flex", flexDirection: "column", cursor: "pointer", padding: "12px", borderRadius: "8px", position: "relative", width: "172px", boxSizing: "border-box" }}
      ...
      >
        <div style={{ position: "relative", marginBottom: "12px" }}>
          <img
            src={book.imageLink}
            alt={book.title}
            style={{ width: "148px", height: "148px", objectFit: "cover", borderRadius: "4px", display: "block" }}
          />
          {book.subscriptionRequired && !isPremiumUser && (
            <div style={{ position: "absolute", top: "8px", right: "24px", backgroundColor: "#032b41", color: "#fff", fontSize: "10px", fontWeight: 600, padding: "2px 8px", borderRadius: "20px" }}>
              Premium
            </div>
          )}
        </div>
        <div style={{ fontSize: "15px", fontWeight: 700, color: "#032b41", marginBottom: "4px", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", maxWidth: "148px" }}>
          {book.title}
        </div>
        <div style={{ fontSize: "13px", color: "#6b757b", marginBottom: "4px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "148px" }}>
          {book.author}
        </div>
        <div style={{ fontSize: "12px", color: "#6b757b", marginBottom: "8px", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", maxWidth: "148px" }}>
          {book.subTitle}
        </div>
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .selected-card { display: flex; gap: 24px; background: #fff3d7; border-radius: 8px; padding: 24px; max-width: 700px; }
      ...
      <section style={{ marginBottom: "40px" }}>
        ...
        {loading ? (
          <div style={{ display: "flex", gap: "16px", overflow: "hidden" }}>
            {[1,2,3,4].map(i => <div key={i} style={{ width: "172px", height: "260px", backgroundColor: "#e8e8e8", borderRadius: "8px", flexShrink: 0 }} />)}
          </div>
        ) : (
